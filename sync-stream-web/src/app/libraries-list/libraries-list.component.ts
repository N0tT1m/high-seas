  ...
  template: `
    <div class="libraries-container">
      ...
      <div class="error-message" *ngIf="error">
        {{ error }}
        <button class="retry-button" (click)="loadLibraries()">Retry</button>
      </div>
      ...
      <div class="libraries-grid" *ngIf="!isLoading && !error">
        <div class="library-card" *ngFor="let library of libraries" (click)="openLibrary(library)">
          ...
          <div class="library-info">
            ...
            <span class="library-id">ID: {{ library.key }}</span>
          </div>
        </div>
      </div>

      <div class="debug-info" *ngIf="libraries.length > 0">
        <h4>Debug Info</h4>
        <pre>{{ libraries | json }}</pre>
      </div>
    </div>
  `,
  styles: [`
