...
@Component({
  selector: 'top-rated-tv-show-details',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
  ...
  template: `
    <article class="earth-spirit" *ngFor="let show of this.fetchedShow?.results; index as i;">
    ...
    `,
  styleUrls: ['./top-rated-tv-shows-details.component.sass', '../../styles.sass'],
})

export class TopRatedTvShowsDetailsComponent {
  ...
  downloadShow(title: string, quality: string) {
    this.tvShowService.makeTvShowDownloadRequest(title, this.seasonEpisodeNumbers, quality, this.tmdbId).subscribe(request => console.log(request));
  }
}
