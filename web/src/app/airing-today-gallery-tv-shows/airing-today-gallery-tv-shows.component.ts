  ...
  <div class="container">
    ...
    <div class="show-names-wrapper">
      <h3 class="name" *ngFor="let show of showNames">
        <a [routerLink]="['/airing-today/shows/details', show['id'], show['page']]">{{ show['name'] }}</a>
      </h3>
    </div>
  </div>
    `,
...
export class AiringTodayGalleryTvShowsComponent {
  ...
  public fetchedShows: TvShow[] = [{ page: 0, results: [{ adult: false, backdrop_path: "", genre_ids: [], id: 0, name: "", first_air_date: "", original_language: "", original_name: "", overview: "", popularity: 0, poster_path: "", vote_average: 0, vote_count: 0, video: false, in_plex: false }], total_pages: 0, total_result: 0 }]
  ...
  ngOnInit() {
    ...
    this.tvShowService.getAiringToday(page).subscribe((resp) => {
      ...
      let page = resp['page'];
        ...
        let result: TvShowResult[] = [{ adult: isAdult, backdrop_path: backdropPath, genre_ids: genreIds, id: id, name: name, first_air_date: firstAirDate, original_language: originalLanguage, original_name: originalName, overview: overview, popularity: popularity, poster_path: posterPath, vote_average: voteAverage, vote_count: voteCount, video: video, in_plex }]
    ...
    // return this.filteredMoviesList = this.allMovies.filter((movie) => movie.results[0]?.original_title.toLowerCase().includes(text.toLowerCase()));
  }
}
