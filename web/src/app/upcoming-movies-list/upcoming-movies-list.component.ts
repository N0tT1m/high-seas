...
@Component({
  ...
  <div>
    <section class="listing" *ngFor="let movie of movie?.results; index as i;">
      <a [routerLink]="['/upcoming/movies/details', movie.id, this.page, this.query]">
      ...
      <a [routerLink]="['/upcoming/movies/details', movie.id, this.page, this.query]">
        <div>
            <h2 class="media-title">{{ movie.title }}</h2>
        </div>
      </a>
  ...
  `,
  styleUrls: ['./upcoming-movies-list.component.sass', '../../styles.sass']
})
export class UpcomingMoviesListComponent {
