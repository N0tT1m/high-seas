...
@Component({
  selector: 'app-discover-anime-details',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
  ...
  <article class="anime-details" *ngFor="let anime of this.fetchedAnime?.results; index as i;">
    <div class="anime-header">
      ...
      <div class="anime-info">
        ...
        <h3 class="anime-original-title" *ngIf="anime!.original_name !== anime!.name">{{anime!.original_name}}</h3>
  ...
  `,
  styleUrls: ['./discover-anime-details.component.sass']
})
export class DiscoverAnimeDetailsComponent {
  ...
  ngOnInit() {
    ...
    const genre = parseInt(this.route.snapshot.params['genre'], 10) || 0;
    const releaseYear = this.route.snapshot.params['releaseYear'];
    const endYear = this.route.snapshot.params['endYear'];

    // Build filter object for discovering anime
    const animeFilters: any = {
      language: 'ja',
      page: page
    };
    ...
    if (releaseYear || endYear) {
      ...
      if (releaseYear) {
        animeFilters.yearRange.start = parseInt(releaseYear);
      }
      if (endYear) {
        animeFilters.yearRange.end = parseInt(endYear);
      }
    }
    ...
    this.tvShowService.discoverShows(animeFilters).subscribe((resp) => {
      if (resp && resp.results) {
        resp.results.forEach((anime) => {
          if (anime.original_language === 'ja') {  // Ensure it's anime (Japanese)
  ...
  downloadAnime(title: string) {
    // For anime, always use the anime download method
