...
@Component({
  selector: 'app-popular-tv-shows-list',
  standalone: true,
  ...
  <div>
    <section class="listing" *ngFor="let show of tvShow?.results; index as i;">
      ...
      <a [routerLink]="['/popular/shows/details', show.id, this.page]">Link to {{ show.name }}</a>
    </section>
  </div>
  `,
  styleUrls: ['./popular-tv-shows-list.component.sass', '../../styles.sass']
})
export class PopularTvShowsListComponent {
